...
class popupPps extends modulePps {
	...
	public function checkPopupShow() {
		...
		if(($currentPageId && $havePostsListing && $isOnlyOnePage) || $isHome) {
            ...
            if(is_front_page()){
                $isOnlyOnePage = true;
                $currentPageId = get_option( 'page_on_front' );
            }
        ...
        //var_dump($currentPageId);
 		...
 		if(!empty($popups)) {
           ...
           $sql_select_rule = "SELECT num FROM ".$wpdb->prefix."cp_statistics_policy WHERE page_id='".$postID."' AND popup_id='".$popup_id."'";
            $dnum = $wpdb->get_col($sql_select_rule);
            $dds = '+'.$dnum[0].' days';
            $expected_show_date = strtotime($dds,$first_visit_timestamp);
            
            $current_timestamp = time();
            
            if($current_timestamp > $expected_show_date){
			$popups = dispatcherPps::applyFilters('popupListBeforeRender', $popups);
			$this->renderList( $popups );
            }
		}
